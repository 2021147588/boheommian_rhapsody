...
static_directory = "/Users/happy/Desktop/boheommian_rhapsody/backend/static"
...
async def submit_data(
    ...
    with open("conversation_analysis_report.json", "r", encoding='utf-8') as file:
        temp = json.load(file)
    ...
    # 결과 출력
    print(f"\n===== 시뮬레이션 결과 =====")
    print(f"총 샘플: {len(temp)}")
    print(f"성공 샘플: {count}")
    print(f"성공률: {(count/len(temp))*100:.1f}%")
    ...
    with open(enhanced_all_file, "w", encoding='utf-8') as file:
        result_data = {
            "summary": {
                ...
                "success_count": {count},
                "success_rate": (count/len(temp))*100,
                "timestamp": timestamp
            },
